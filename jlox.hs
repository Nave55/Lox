...
scanToken loc scanner =
  let (n_loc, c) = advance loc scanner
      (n_loc', scanner') = case c of
        '('  -> (n_loc, addToken LEFT_PAREN  n_loc scanner)
        ')'  -> (n_loc, addToken RIGHT_PAREN n_loc scanner)
        '{'  -> (n_loc, addToken LEFT_BRACE  n_loc scanner)
        '}'  -> (n_loc, addToken RIGHT_BRACE n_loc scanner)
        ','  -> (n_loc, addToken COMMA       n_loc scanner)
        '.'  -> (n_loc, addToken DOT         n_loc scanner)
        '-'  -> (n_loc, addToken MINUS       n_loc scanner)
        '+'  -> (n_loc, addToken PLUS        n_loc scanner)
        ';'  -> (n_loc, addToken SEMICOLON   n_loc scanner)
        '*'  -> (n_loc, addToken STAR        n_loc scanner)
        '\n' -> (n_loc { l_line = l_line n_loc + 1 }, scanner)

        u_val ->
          let err = createError (l_line n_loc) ("Unexpected Character '" ++ [u_val] ++ "'")
          in (n_loc, scanner { s_errors = err : s_errors scanner })
  in (n_loc', scanner')
...
scanTokens loc scanner
  | isAtEnd (l_current loc) (s_source scanner) =
      let eofTok = createToken EOF "" L_NIL (l_line loc)
          scanner' = scanner { s_tokens = eofTok : s_tokens scanner }
...
scanToken loc scanner =
  let (n_loc, c) = advance loc scanner
      (n_loc', scanner')   = case c of
        ...
        '*'  -> (n_loc, addToken STAR        n_loc scanner)
        '\n' ->
          -- let err = createError (l_line n_loc) "Unexpected Character '\\n'"
          (n_loc { l_line = l_line n_loc + 1 }, scanner)
